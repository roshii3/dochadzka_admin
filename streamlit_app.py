...
def get_user_pairs(pos_day_df: pd.DataFrame):
    ...
    for user in pos_day_df["user_code"].unique():
        ...
        pairs[user] = {
            "pr": pr.min() if not pr.empty else pd.NaT,
...
def classify_pair(pr, od, position):
    msgs = []
    if pd.isna(pr) and pd.isna(od): return ("none","none",0.0,0.0,msgs)
    if pd.isna(pr): msgs.append("missing_prichod"); return ("missing_pr", "none",0.0,0.0,msgs)
    if pd.isna(od): msgs.append("missing_odchod"); return ("none","missing_od",0.0,0.0,msgs)
...
def summarize_position_day(pos_day_df: pd.DataFrame, position):
    ...
    for user, pair in pairs.items():
        ...
        if msgs:
            for m in msgs: details.append(f"{user}: {m} — pr:{pair['pr']} od:{pair['od']}")
    return morning, afternoon, details
    ...
    for pos in df_raw["position"].unique():
        ...
        for i in range(7):
            ...
            for user,pair in pairs.items():
                ...
                pr_t, od_t = pair["pr"].time(), pair["od"].time()
...
def excel_with_colors(df_matrix, df_day_details, df_raw, monday):
    ...
    for r in dataframe_to_rows(df_day_details,index=False,header=True): ws2.append(r)
    ws3=wb.create_sheet("Surové dáta")
    for r in dataframe_to_rows(df_raw,index=False,header=True): ws3.append(r)
...
if df_week.empty: st.warning("Rozsah nie je dostupný v DB (žiadne dáta pre vybraný týždeň).")
else:
    ...
    if st.button("Exportuj Excel (Farebné)"):
        ...
        df_raw=df_week.copy()
        if "timestamp" in df_raw.columns: df_raw["timestamp"]=df_raw["timestamp"].apply(lambda x: x.isoformat() if pd.notna(x) else "")
