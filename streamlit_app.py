...
def summarize_position_day(pos_day_df: pd.DataFrame, position):
    ...
    for user, pair in pairs.items():
        ...
        if role_m == "R+P OK" and role_p == "R+P OK":
            morning = {"status": "R+P OK", "hours": h_m, "detail": f"{user}: {pair['pr']} - {pair['od']}"}
            afternoon = {"status": "R+P OK", "hours": h_p, "detail": f"{user}: {pair['pr']} - {pair['od']}"}
            return morning, afternoon, details
        if role_m == "Ranna OK":
...
# ================== FUNKCIA NA EXCEL REPORT ==================

def vygeneruj_excel_report(df_matrix, df_day_details, df_raw, monday):
    wb = Workbook()
    ws1 = wb.active
...
# --- Denný prehľad ---
selected_day = st.sidebar.date_input("Denný prehľad - vyber deň", value=today)
...
if st.button("Exportuj Excel (Farebné)"):
    ...
    xls = vygeneruj_excel_report(df_matrix, df_day_details, df_raw, monday)
    st.download_button(
        "Stiahnuť XLSX",
        data=xls,
