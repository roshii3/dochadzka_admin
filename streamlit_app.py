...
def classify_pair(pr, od, position):
    ...
    # Veliteľ special: can come early (03:00) and leave late => counts as double
...
def summarize_position_day(pos_day_df: pd.DataFrame, position):
    ...
    if rp_user:
        ...
        morning = {
            "status": "R+P OK",
            "hours": h_m,
            "detail": f"Príchod: {pair['pr']}, Odchod: {pair['od']}"
        }
        afternoon = {
            "status": "R+P OK",
            "hours": h_p,
    ...
    # otherwise look for separate morning/afternoon
    for user, pair in pairs.items():
        role_m, role_p, h_m, h_p, msgs = classify_pair(pair["pr"], pair["od"], position)
    ...
    # adjust totals if different people cover morning and afternoon
